...
print("rho(H_D) =", spectralRadius(hd))
print()
...
print("lambda_n / lambda_n-1 =", wa[n - 1] / wa[n - 2])
print()
...
hl = np.tril(hd, -1)
hr = np.triu(hd)
print("pho(H_S) =", spectralRadius(np.linalg.inv(np.identity(n, float) - hl).dot(hr)))
print()
...
print("rho(S_q) =", 2 / (1 + sqrt(1 - spectralRadius(hd))) - 1)